...
print 'Anansi::ComponentManager->addComponent'."\n";
print Data::Dumper::Dumper(@parameters);

                $OBJECT = Anansi::Actor->new(
                    PACKAGE => $component,
                    PARAMETERS => [(@parameters)],
                );
                last;
...
=head2 components
    ...
    if(ref($components) =~ /^ARRAY$/i) {
        foreach my $component (@{$components}) {
            print 'Component namespace: "'.$component.'"'."\n";
        }
    }
...
Either returns an array of strings containing the namespace of all of the
available components or the current component manager's components.
...
I<PRIORITY_OF_VALIDATE> component channel.
...
Either returns the priority of the component that has highest priority, the list
of all the component namespaces that have the priority supplied as the
I<PRIORITY> parameter or the priority of the component given it's namespace
supplied as the I<PRIORITY> parameter.  Each priority is represented by a number
of B<0> I<(zero)> or greater with greater values implying higher priority.
Utilises the I<PRIORITY_OF_VALIDATE> component channel as the source of
priority.
