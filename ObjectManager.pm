...
sub identification {
    ...
    if(!defined($instance)) {
        ...
        if($instance =~ /^\s*$/) {
        } elsif(!defined($self->{IDENTIFICATIONS})) {
        } elsif(ref($self->{IDENTIFICATIONS}) =~ /^ARRAY$/i) {
            for(my $index = 0; $index < scalar(@{$self->{IDENTIFICATIONS}}); $index++) {
                return $index if($instance == @{$self->{IDENTIFICATIONS}}[$index]);
            }
            if($instance !~ /^\d+$/) {
            } elsif(0 + $instance < scalar(@{$self->{IDENTIFICATIONS}})) {
        ...
        } elsif(!defined(${$instance->{Anansi}}{ObjectManager})) {
        } elsif(ref(${$instance->{Anansi}}{ObjectManager}) !~ /^HASH$/i) {
        } elsif(!defined(${${$instance->{Anansi}}{ObjectManager}}{IDENTIFICATION})) {
...
print 'Anansi::ObjectManager->register'."\n";
...
# -- fudge code --
    if(!defined($instance->{IDENTIFICATION})) {
        $instance->{IDENTIFICATION} = ${${$instance->{Anansi}}{ObjectManager}}{IDENTIFICATION};
    }
# -- current code --
